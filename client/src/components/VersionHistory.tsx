    ...
    <div>
      ...
      <div className="p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={handleQuality} className="text-[10px] text-emerald-400 hover:text-emerald-300">
            检查
          </button>
        </div>
        ...
        {session.snapshots.length === 0 ? (
          ...
          <div className="space-y-2">
            {[...session.snapshots].reverse().map((snapshot) => (
              <div key={snapshot.version} className="p-2.5 bg-gray-900/50 border border-gray-800 rounded-lg">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-mono text-emerald-400">v{snapshot.version}</span>
        ...
        <div className="mt-6 pt-4 border-t border-gray-800">
          ...
          <div className="space-y-2">
            {session.openQuestions.filter((item) => item.status === 'open').map((item) => (
              <div key={item.id} className="text-xs text-gray-500 leading-relaxed bg-gray-900/30 rounded p-2">
                {item.question}
              </div>
            ))}
          </div>
        ...
        <div className="mt-6 pt-4 border-t border-gray-800">
          ...
          <div className="space-y-1.5">
            {['/review', '/generate-tech', '/generate-acceptance', '/quality', '/freeze', '/reset'].map((cmd) => (
              <div key={cmd} className="px-2.5 py-1.5 bg-gray-900/30 rounded text-xs">
